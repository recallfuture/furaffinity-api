...
function checkSystemMessage($: CheerioStatic) {
  ...
  if (noticeMessage.length !== 0) {
    const systemMessage =
      noticeMessage[0].childNodes[1].childNodes[3].childNodes[0].nodeValue
...
export function ParseFigure(
  figure: CheerioElement,
  selector: Cheerio
): IResult {
  const id: string = figure.attribs.id.split("-").pop() ?? ""
  const thumb: string =
    "http:" +
    figure.childNodes[0].childNodes[0].childNodes[0].childNodes[0].attribs.src
  ...
  return {
    ...
    id,
    title:
      figure.childNodes[1].childNodes[0].childNodes[0].childNodes[0]
        ?.nodeValue ?? "",
...
export function ParseSubmissionsPaging(
  body: string,
  results: IPagingResults
): IPagingResults {
  const $ = cheerio.load(body)
...
export function ParseSubmission(body: string, id: string): ISubmission {
  ...
  // buttons
  let downloadUrl: string = `http:${
    sidebar.find(".buttons .download a")[0].attribs.href
  }`
  ...
  const authorName: string =
    content
      .find(".submission-id-sub-container a strong")[0]
      .childNodes[0].data?.trim() ?? ""
  const authorId: string = convertNameToId(authorName)
  const posted: string = content.find(
    ".submission-id-sub-container strong span"
  )[0].attribs.title
  const authorAvatar: string = `http:${
    content.find(".submission-id-avatar img")[0].attribs.src
  }`
  ...
  const rating: Rating =
    Rating[
      stats
        .find(".rating span")[0]
        .childNodes[0].data?.trim() as keyof typeof Rating
    ]
  ...
  const category: Category =
    Category[
      info
        .find(".category-name")[0]
        .childNodes[0].data?.trim() as keyof typeof Category
    ]
  const species: Species =
    Species[
      info[0].childNodes[3].childNodes[2].childNodes[0].data?.trim() as keyof typeof Species
    ]
  const gender: Gender =
    Gender[
      info[0].childNodes[5].childNodes[2].childNodes[0].data?.trim() as keyof typeof Gender
    ]
  ...
  const previewUrl: string | undefined =
    content.find(".submission-area img").length > 0
      ? `http:${
          content.find(".submission-area img")[0].attribs["data-preview-src"]
        }`
      : undefined
...
export function ParseAuthor(body: string): IAuthor {
  ...
  const shinies: boolean = !!$(
    ".userpage-layout-left-col-content > a:nth-child(4)"
  )
